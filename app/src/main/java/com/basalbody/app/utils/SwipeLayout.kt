...
class SwipeLayout(context: Context, attrs: AttributeSet?) : FrameLayout(context, attrs) {
    ...
    override fun onInterceptTouchEvent(event: MotionEvent): Boolean {
        if (disallowIntercept && isViewGroup(draggedView)) {
            val neededScrollView =
                getNeededTouchView(event, (draggedView as android.view.ViewGroup?)!!)
            val touchPoint = Point(event.getX().toInt(), event.getY().toInt())
    ...
    override fun onFinishInflate() {
        if (draggedViewId != 0) {
            draggedView = findViewById<View>(draggedViewId)
        }

        if (staticLeftViewId != 0) {
            staticLeftView = findViewById<View?>(staticLeftViewId)
        }

        if (staticRightViewId != 0) {
            staticRightView = findViewById<View?>(staticRightViewId)
        }
        ...
        gestureDetector = GestureDetectorCompat(getContext(), gestureDetectorCallBack)
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (isSwipeViewTarget(event) || this.isMoving) {
    ...
    fun openRight(animated: Boolean) {
        if (animated) {
            openRight()
        } else if (isDragIdle(currentDraggingState) && ((currentDirection == LEFT && !this.isEmptyRightView)
                    || currentDirection == HORIZONTAL) && !isRightOpen
        ) {
            if (isTogether) {
                staticRightView!!.offsetLeftAndRight(-1 * (if (isLeftOpen) this.rightViewWidth * 2 else this.rightViewWidth))
            }

            draggedView!!.offsetLeftAndRight(-1 * (if (isLeftOpen) this.rightViewWidth * 2 else this.rightViewWidth))
            draggingViewLeft -= (if (isLeftOpen) this.rightViewWidth * 2 else this.rightViewWidth)
            updateState()
        }
    }
    ...
    fun openLeft(animated: Boolean) {
        if (animated) {
            openLeft()
        } else if (isDragIdle(currentDraggingState) && ((currentDirection == RIGHT && !this.isEmptyLeftView)
                    || currentDirection == HORIZONTAL) && !isLeftOpen
        ) {
            if (isTogether) {
                staticLeftView!!.offsetLeftAndRight((if (isRightOpen) this.leftViewWidth * 2 else this.leftViewWidth))
            }

            draggedView!!.offsetLeftAndRight((if (isRightOpen) this.leftViewWidth * 2 else this.leftViewWidth))
            draggingViewLeft += (if (isRightOpen) this.leftViewWidth * 2 else this.leftViewWidth)
            updateState()
        }
    }
     ...
     */
    fun openLeft() {
        if (isDragIdle(currentDraggingState) && ((currentDirection == RIGHT && !this.isEmptyLeftView)
                    || currentDirection == HORIZONTAL)
        ) {
            moveTo(this.leftViewWidth)
        }
    }
     ...
     */
    fun openRight() {
        if (isDragIdle(currentDraggingState) && ((currentDirection == LEFT && !this.isEmptyRightView)
                    || currentDirection == HORIZONTAL)
        ) {
            moveTo(-this.rightViewWidth)
        }
    }
    ...
    private fun updateState() {
        if (this.isClosed) {
            ...
            if (actionsListener != null) {
                actionsListener!!.onOpen(RIGHT, this.isLeftOpenCompletely)
            }
            ...
            if (actionsListener != null) {
                actionsListener!!.onOpen(LEFT, this.isRightOpenCompletely)
            }
        }
    }
    ...
    private fun clampLeftViewPosition(left: Int): Int {
        if (isContinuousSwipe && this.isEmptyRightView) {
            if (isFreeHorizontalDrag) {
                return if (left > horizontalWidth) CLOSE_POSITION else max(left, -horizontalWidth)
            } else {
                return if (left > CLOSE_POSITION) CLOSE_POSITION else max(left, -horizontalWidth)
            }
        }

        if (isFreeDragAfterOpen) {
            if (isFreeHorizontalDrag) {
                return if (left > horizontalWidth)
                    CLOSE_POSITION
                else max(left, leftDragViewPadding - horizontalWidth)
            }
            return if (left > CLOSE_POSITION)
                CLOSE_POSITION
            else max(left, leftDragViewPadding - horizontalWidth)
        }

        if (isFreeHorizontalDrag) {
            return if (left > horizontalWidth) CLOSE_POSITION else max(left, -this.rightViewWidth)
        }
        return if (left > CLOSE_POSITION) CLOSE_POSITION else max(left, -this.rightViewWidth)
    }

    private fun clampRightViewPosition(left: Int): Int {
        if (isContinuousSwipe && this.isEmptyLeftView) {
            if (isFreeHorizontalDrag) {
                return if (left < -horizontalWidth) -horizontalWidth else min(left, horizontalWidth)
            } else {
                return if (left < CLOSE_POSITION) CLOSE_POSITION else min(left, horizontalWidth)
            }
        }
        ...
        if (isFreeDragAfterOpen) {
            if (isFreeHorizontalDrag) {
                return if (left < -horizontalWidth)
                    -horizontalWidth
                else min(left, horizontalWidth - rightDragViewPadding)
            }
            return if (left < CLOSE_POSITION)
                CLOSE_POSITION
            else min(left, horizontalWidth - rightDragViewPadding)
    ...
    private fun clampHorizontalViewPosition(left: Int, dx: Int): Int {
        if (!isFreeHorizontalDrag && isLeftOpen && dx < 0) {
            return max(left, CLOSE_POSITION)
        }

        if (!isFreeHorizontalDrag && isRightOpen && dx > 0) {
            return min(left, CLOSE_POSITION)
        }

        if (!isFreeDragAfterOpen && left > CLOSE_POSITION) {
            return min(left, this.leftViewWidth)
        }

        if (!isFreeDragAfterOpen && left < CLOSE_POSITION) {
            return max(left, -this.rightViewWidth)
        }

        return if (left < CLOSE_POSITION) max(left, leftDragViewPadding - horizontalWidth) else min(
            left,
    ...
    private fun getFinalXLeftDirection(xVel: Float): Int {
        if (isContinuousSwipe) {
            if (this.isEmptyRightView) {
                if ((draggingViewLeft < CLOSE_POSITION && abs(draggingViewLeft) > horizontalWidth / 2)
                    || xVel < -autoOpenSpeed
                ) {
                    return -horizontalWidth
                }
        ...
        if (xVel > autoOpenSpeed) {
            ...
            settleToOpen = true
        } else if (draggingViewLeft < CLOSE_POSITION && abs(draggingViewLeft) > this.rightViewWidth / 2) {
            settleToOpen = true
        } else if (draggingViewLeft < CLOSE_POSITION && abs(draggingViewLeft) < this.rightViewWidth / 2) {
            settleToOpen = false
        } else {
        ...
        return if (settleToOpen) -this.rightViewWidth else CLOSE_POSITION
    }
    ...
    private fun getFinalXRightDirection(xVel: Float): Int {
        ...
        if (xVel > autoOpenSpeed) {
            ...
            settleToOpen = false
        } else if (draggingViewLeft > CLOSE_POSITION && abs(draggingViewLeft) > this.leftViewWidth / 2) {
            settleToOpen = true
        } else if (draggingViewLeft > CLOSE_POSITION && abs(draggingViewLeft) < this.leftViewWidth / 2) {
            settleToOpen = false
        } else {
        ...
        return if (settleToOpen) this.leftViewWidth else CLOSE_POSITION
    }
    ...
    private fun isContinuousSwipeToRight(xVel: Float): Boolean {
        return (xVel > autoOpenSpeed && abs(draggingViewLeft) > this.leftViewWidth)
                || (draggingViewLeft > CLOSE_POSITION && abs(draggingViewLeft) > horizontalWidth / 2)
    }

    private fun isContinuousSwipeToLeft(xVel: Float): Boolean {
        return (xVel < -autoOpenSpeed && abs(draggingViewLeft) > this.rightViewWidth)
                || (draggingViewLeft < CLOSE_POSITION && abs(draggingViewLeft) > horizontalWidth / 2)
    }
    ...
    private fun isSwipeToOpenRight(xVel: Float): Boolean {
        ...
        return (draggingViewLeft < CLOSE_POSITION && xVel < -autoOpenSpeed)
                || (draggingViewLeft < CLOSE_POSITION && abs(draggingViewLeft) > this.rightViewWidth / 2)
    }
    ...
    private fun isSwipeToOpenLeft(xVel: Float): Boolean {
        ...
        return (draggingViewLeft > CLOSE_POSITION && xVel > autoOpenSpeed)
                || (draggingViewLeft > CLOSE_POSITION && abs(draggingViewLeft) > this.leftViewWidth / 2)
    }
    ...
    private fun setupPost() {
        if (isTogether) {
            post(object : Runnable {
                override fun run() {
                    if (currentDirection == LEFT) {
                        staticRightView!!.setX(horizontalWidth.toFloat())
                    } else if (currentDirection == RIGHT) {
                        staticLeftView!!.setX(-staticLeftView!!.getWidth().toFloat())
                    } else if (currentDirection == HORIZONTAL) {
                        staticRightView!!.setX(horizontalWidth.toFloat())
                        staticLeftView!!.setX(-staticLeftView!!.getWidth().toFloat())
                    }
    ...
    private fun getNeededTouchView(event: MotionEvent?, rootView: ViewGroup): View? {
        ...
        val count = rootView.getChildCount()
    ...
    private fun isSwipeViewTarget(event: MotionEvent): Boolean {
        ...
        val upperLimit = swipeViewLocation[1] + draggedView!!.getMeasuredHeight()
    ...
    private val isRightViewOpen: Boolean
        get() = staticRightView != null && draggingViewLeft == -this.rightViewWidth

    private val isLeftViewOpen: Boolean
        get() = staticLeftView != null && draggingViewLeft == this.leftViewWidth
    ...
    private val leftViewWidth: Int
        get() = staticLeftView!!.getWidth()

    private val rightViewWidth: Int
        get() = staticRightView!!.getWidth()
